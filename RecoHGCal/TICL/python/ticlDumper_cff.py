        ...
        dumperAssociators.append(
            cms.PSet(
                ...
                associatorRecoToSimInputTag=cms.InputTag(
                    f"allTrackstersToSimTrackstersAssociationsByLCs:{tracksterIteration}To{simTrackstersCollection}"
                ),
                associatorSimToRecoInputTag=cms.InputTag(
                    f"allTrackstersToSimTrackstersAssociationsByLCs:{simTrackstersCollection}To{tracksterIteration}"
                ),
            )
        )
...
ticlDumper = ticlDumper_.clone(
    tracksterCollections=[
        *[cms.PSet(treeName=cms.string(label), inputTag=cms.InputTag(label)) for label in ticlIterLabels],
        ...
        cms.PSet(
            treeName=cms.string("ticlTracksterLinksPre"),
            inputTag=cms.InputTag("ticlTracksterLinksPre"),
            tracksterType=cms.string("Trackster"),
        ),
...
(ticl_v5 & ticl_superclustering_mustache_pf).toModify(
    ticlDumper,
